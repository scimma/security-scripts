...
def main(args):
   ...
   # expected folder structure: "logs/Scimma-event-trail/AWSLogs/585193511743/CloudTrail/"
   # TODO: make acct id dynamic? 585193511743

   # regex the bucket and prefix
   try:
       re_bucket = re.findall('(?<=s3:\/\/)(.*?)(?=\/)', args.bucket)[0]
       re_prefix = re.findall('([^\/]+$)', args.bucket)[0] + '/AWSLogs/585193511743/CloudTrail/' # TODO: i don't like this
   except IndexError:
